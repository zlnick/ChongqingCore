...
| 卫生机构（组织）代码 | 0..1 | 卫生机构（组织）代码 | 卫生机构（组织）代码遵循WS 218-2002 卫生机构（组织）分类与代码标准要求，为22位(不含连字符)或者23位代码（含连字符）字符串，其中只包括数字，大写字母和连字符'-' | [Organization.identifier:hcoc](StructureDefinition-hc-healthcare-organization-definitions.html#Organization.identifier:hcoc) |
